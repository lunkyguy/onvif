...
static int serve___name__GetServiceCapabilities(AdvancedSecurityServiceBindingService*);
...
{	AdvancedSecurityServiceBindingService_init(this->soap->imode, this->soap->omode);
	...
	if (!soap_match_tag(this->soap, this->soap->tag, "ns5:GetServiceCapabilities"))
		return serve___name__GetServiceCapabilities(this);
	return this->soap->error = SOAP_NO_METHOD;
}

static int serve___name__GetServiceCapabilities(AdvancedSecurityServiceBindingService *service)
{	struct soap *soap = service->soap;
	struct __name__GetServiceCapabilities soap_tmp___name__GetServiceCapabilities;
	...
	soap_default___name__GetServiceCapabilities(soap, &soap_tmp___name__GetServiceCapabilities);
	if (!soap_get___name__GetServiceCapabilities(soap, &soap_tmp___name__GetServiceCapabilities, "-name:GetServiceCapabilities", NULL))
		return soap->error;
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
	 || soap_end_recv(soap))
		return soap->error;
	soap->error = service->GetServiceCapabilities(soap_tmp___name__GetServiceCapabilities.ns5__GetServiceCapabilities, &ns5__GetServiceCapabilitiesResponse);
	if (soap->error)
		return soap->error;
